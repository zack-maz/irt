...
const WRI_URLS = ['https://files.wri.org/aqueduct/aqueduct-4-0-water-risk-data.zip'];
  ...
  try {
    ...
    if (downloaded) {
      ...
      if (csvEntry) {
        ...
        const geoPackage = entries.find((e) => e.entryName.toLowerCase().endsWith('.gpkg'));
        if (geoPackage) {
...
function parseAqueductCSV(csvContent: string): BasinEntry[] {
  ...
  console.log(
    `Column mapping: pfaf=${pfafCol}, name=${nameCol}, bws_raw=${bwsRawCol}, bws_score=${bwsScoreCol}, lat=${latCol}, lng=${lngCol}`,
  );
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const inBbox =
      lat >= 0 &&
      lng >= 0 &&
      lat >= FILTER_BBOX.latMin &&
      lat <= FILTER_BBOX.latMax &&
      lng >= FILTER_BBOX.lngMin &&
      lng <= FILTER_BBOX.lngMax;
