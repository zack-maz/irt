...
| PRES-06                                    | `.env.example` lists every env var consumed by `server/config.ts` with a one-line description and example value shape; no stale or missing entries                                                                                                                                                                            | `.env.example` currently lists ACLED (deprecated) and omits LOG*LEVEL, NODE_ENV, EVENT*\* tuning knobs                                                                                                    |
...
```json
{
  ...
  "ignore": ["src/test/**", "server/__tests__/**", "**/*.test.{ts,tsx}", "src/data/**"],
