...
tech-stack:
  ...
  patterns:
    [
      'Promise.race route-level timeout pattern for unreliable upstream APIs',
      'Sequential batch splitting for large Overpass queries with partial-success fallback',
    ]
...
- **Issue:** 6 existing tests in src/**tests**/waterStress.test.ts expected old black [0,0,0] color values
- **Fix:** Updated assertions to match new dark-purple [40,20,60] floor and recalculated midpoint colors
- **Files modified:** src/**tests**/waterStress.test.ts
