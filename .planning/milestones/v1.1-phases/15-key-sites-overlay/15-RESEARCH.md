...
| Client-side brute force attack check | R-tree spatial index      | Brute force O(sites _ events) is fine for ~100-200 sites _ ~1000 events; spatial index adds complexity for no measurable gain |
...
- Project codebase: Existing adapter/route/store/layer patterns (server/adapters/_.ts, server/routes/_.ts, src/stores/\*.ts, src/hooks/useEntityLayers.ts)
